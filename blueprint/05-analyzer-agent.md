...
## Overview
The Analyzer Agent processes tool execution results to extract insights, detect anomalies, identify patterns, and extract entities. It transforms raw data into meaningful information for the Summarizer.
...
import { ToolResult, Analysis, Insight, Anomaly, Entity } from './types.js';
...
export class AnalyzerAgent {
  ...
  async analyze(results: ToolResult[]): Promise<Analysis> {
    console.log(`Analyzing ${results.length} tool results`);
...
describe('AnalyzerAgent', () => {
  ...
  it('should analyze shipment results', async () => {
    const results = [{
      success: true,
      tool: 'shipments',
      data: [
        ...
        { id: 'S3', has_contaminants: false, status: 'accepted' }
      ],
  ...
  it('should detect high contamination rates', async () => {
    const results = [{
      success: true,
      tool: 'shipments',
  ...
  it('should extract entities with relationships', async () => {
    const results = [{
      success: true,
      tool: 'contaminants-detected',
      data: [{
...
1. Implement rule-based analysis first
2. Add statistical analysis methods
3. Add LLM-based analysis
4. Add entity extraction
5. Add relationship detection
6. Add anomaly detection algorithms
7. Add learning from feedback
